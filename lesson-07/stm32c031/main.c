...
int main(void) {
    ...
    // GPIOA MODER
    GPIOA[0]   &= ~((3U << 2U*LD4_PIN) | (3U << 2U*LD5_PIN));
    GPIOA[0]   |=  ((1U << 2U*LD4_PIN) | (1U << 2U*LD5_PIN));
    // GPIOA OTYPER
    GPIOA[1]   &= ~((1U <<    LD4_PIN) | (1U <<    LD5_PIN));
    // GPIOA OSPEEDR
    GPIOA[2]   &= ~((3U << 2U*LD4_PIN) | (3U << 2U*LD5_PIN));
    GPIOA[2]   |=  ((1U << 2U*LD4_PIN) | (1U << 2U*LD5_PIN));
    // GPIOA PUPDR
    GPIOA[3]   &= ~((3U << 2U*LD4_PIN) | (3U << 2U*LD5_PIN));

    GPIOA[6] = (1U << LD5_PIN); // turn LD5 on

    while (1) { // endless loop
        GPIOA[6] = (1U << LD4_PIN); // turn LD4 on
        ...
        GPIOA[6] = (1U << (LD4_PIN + 16U)); // turn LD4 off
        counter = 0;
        while (counter < 500000) {  // delay loop
            ++counter;
        }
