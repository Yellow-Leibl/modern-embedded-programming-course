...
volatile uint32_t y = LD4_PIN | LD5_PIN;

volatile int16_t sqr[] = {
    1*1,
    2*2,
    3*3,
    4*4
...
int main(void) {
    volatile int16_t  _kx = x;
    volatile uint32_t _ky = y;
    volatile int16_t  _ks = sqr[0] + sqr[1] + sqr[2] + sqr[3];
    (void)_kx; (void)_ky; (void)_ks;
...
//............................................................................
// Required by Samek-style C startup: every fault handler ends up calling
// assert_failed(). Here we simply trigger a controlled software reset so
// the system recovers cleanly instead of locking up in an infinite loop.
_Noreturn void assert_failed(char const * const module, int const id);
_Noreturn void assert_failed(char const * const module, int const id) {
    (void)module;   // avoid unused-parameter warning
    (void)id;
    NVIC_SystemReset();
    for (;;) { }    // unreachable, but keeps _Noreturn happy
}
