...
/* The context switch, written in inline assembly.
 * #if (__ARM_ARCH == 6) selects the Cortex-M0+ (ARMv6-M) path, where PUSH/POP
 * cannot target r8-r11 — so we shuffle them through r4-r7 with STMIA/LDMIA. */
__attribute__ ((naked))
void PendSV_Handler(void) {
__asm volatile (
    /* GCC emits `.syntax divided` before naked inline asm; force UAL so the
     * `ADDS Rd,#imm8` form (and other unified mnemonics) assemble correctly. */
    "  .syntax unified                 \n"
    ...
    "  MOV           r0,sp             \n" // r0 := top of stack
    "  MOV           r2,r0             \n"
    "  ADDS          r2,#(4*4)         \n" // point r2 to the 4 high registers r8-r11 (2-op form for GNU as)
